...
artists.each do |artist|
    ...
    new_artist.photo.attach(
        io: URI.open("https://music-panel-seeds.s3.amazonaws.com/#{artist_path}-photo.jpeg"),
        filename: "#{artist_path}-photo.jpeg"
    )

    new_artist.background.attach(
        io: URI.open("https://music-panel-seeds.s3.amazonaws.com/#{artist_path}-background.jpeg"),
        filename: "#{artist_path}-background.jpeg"
    )
    
    new_artist.small_background.attach(
        io: URI.open("https://music-panel-seeds.s3.amazonaws.com/#{artist_path}-background-small.jpeg"),
    ...
    artist["albums"].each do |album|
        ...
        new_album = Album.create!(album.except("path", "tracks", "background"))
        album_path = album["path"]

        new_album.cover.attach(
            io: URI.open("https://music-panel-seeds.s3.amazonaws.com/#{album_path}-cover.jpeg"),
            filename: "#{album_path}-cover.jpeg"
        )
    
        if album["background"]
            new_album.background.attach(
                io: URI.open("https://music-panel-seeds.s3.amazonaws.com/#{album_path}-background.jpeg"),
                filename: "#{album_path}-background.jpeg"
            )
            
            new_album.small_background.attach(
                io: URI.open("https://music-panel-seeds.s3.amazonaws.com/#{album_path}-background-small.jpeg"),
